...
### 8.2 Set the target SaaS

Edit [infra/main.bicepparam](infra/main.bicepparam) and set:

```bicep
param saasUrl = 'https://<your-saas-host>/'
```

This is the URL the kiosk Chromium opens with `--app=<URL>`. Everything else (broker, sandbox, network) is SaaS-agnostic — see §5 for how uploads work for any SaaS that uses an HTML file picker.

### 8.3 Three-command deploy
...
1. Validates and runs the Bicep at subscription scope (creates RG, VNet + NAT GW, ACA env, broker Container App, Front Door Premium + WAF, Log Analytics + App Insights — ~10–15 min).
2. Auto-approves the Front Door shared Private Link to the ACA env.
3. Idempotently seeds `broker-session-secret` in Key Vault and force-restarts the broker if a Key Vault exists in the RG (skipped otherwise; the broker falls back to a per-deploy secret generated by Bicep `newGuid()`).
4. Polls `https://<frontDoorEndpoint>/healthz` until it returns 200 (up to 25 min — first-time Front Door probe convergence after the freshly-approved Private Link can take 15–20 min).
...
### 8.4 Lock down (before handing the URL to users)
...
### 8.6 Building images from source (optional)
